...
$(document).ready(function() {
    ...
    TeamsStrava.message = function(text) {
        $('#messages').fadeOut('slow', function() {
            $('#messages').fadeIn('slow').html(text)
        });
    };

    TeamsStrava.errorMessage = function(message) {
        TeamsStrava.message(message)
        $('#messages').addClass('has-error');
    };

    TeamsStrava.error = function(xhr) {
        try {
            var message;
            if (xhr.responseText) {
            ...
            TeamsStrava.errorMessage(message || xhr.statusText || xhr.responseText || 'Unexpected Error');

        } catch (err) {
            TeamsStrava.errorMessage(err.message);
        }
    };
});