  ...
  context 'with a STRAVA_CLIENT_SECRET' do
    ...
    context 'webhook event' do
      ...
      context 'with a connected user' do
        ...
        before do
          allow_any_instance_of(User).to receive(:left_team?).and_return(false)
        end
        ...
        context 'with a user that left the team' do
          before do
            allow_any_instance_of(User).to receive(:left_team?).and_return(true)
          end

          it 'disconnects the user instead of syncing' do
            expect_any_instance_of(Logger).to receive(:info).with(/left the team/).and_call_original
        ...
        context 'with an expired subscription' do
          ...
          it 'does not sync user' do
            ...
            expect_any_instance_of(User).not_to receive(:sync_activity_and_brag!).once
            post '/api/strava/event',
                 JSON.dump(
                   event_data.merge(
                     aspect_type: 'create',
          ...
          context 'a bragged activity' do
            ...
            context 'with multiple connected users with the same athlete id' do
              ...
              it 'skips over failures' do
                activities = []
                allow_any_instance_of(UserActivity).to receive(:unbrag!) do |a_activity, _args|
                  raise 'error' if a_activity._id == activity._id
                ...
                response = JSON.parse(last_response.body)
                expect(response['ok']).to be true
                expect(activities).to eq([activity])
              end
