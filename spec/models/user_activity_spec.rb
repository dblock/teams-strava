...
describe UserActivity do
  ...
  context 'brag!' do
    ...
    it 'sends a message to the subscribed channel' do
      expect(TeamsStrava::Bot.instance).to receive(:send_message).with(
        user.channel_id,
        user.service_url,
        activity.to_message(user.channel_id)
      ).and_return(Struct.new(:id).new('1'))
      expect(activity.brag!).to eq(message_id: '1', channel_id: user.channel_id)
    end
    ...
    it 'disables user sync on missing permissions error' do
      expect(TeamsStrava::Bot.instance).to receive(:send_message).with(
        ...
        user.service_url,
        activity.to_message(user.channel_id)
      ).and_raise(TeamsStrava::Error, 'Forbidden (403)')
      expect(activity.brag!).to be_nil
      expect(activity.bragged_at).not_to be_nil
      expect(user.reload.sync_activities).to be false
    end
    ...
    it 'disables user sync on unknown channel error' do
      expect(TeamsStrava::Bot.instance).to receive(:send_message).with(
        ...
        user.service_url,
        activity.to_message(user.channel_id)
      ).and_raise(TeamsStrava::Error, 'Unknown Conversation (404)')
      expect(activity.brag!).to be_nil
      expect(activity.bragged_at).not_to be_nil
      expect(user.reload.sync_activities).to be false
    end
    ...
    context 'with a max_activities_per_channel_per_day limit' do
      ...
      it 'marks the activity bragged without posting it again' do
        expect(TeamsStrava::Bot.instance).not_to receive(:send_message)
        expect(activity.brag!).to be_nil
    ...
    context 'with a channel activity type filter' do
      ...
      context 'activity type matches' do
        ...
        it 'sends a message' do
          expect(TeamsStrava::Bot.instance).to receive(:send_message).and_return(Struct.new(:id).new('1'))
          expect(activity.brag!).to eq(message_id: '1', channel_id: user.channel_id)
        end
      end
  ...
  context 'rebrag!' do
    ...
    it 'clears channel message on unknown message error' do
      expect(TeamsStrava::Bot.instance).to receive(:update_message).and_raise(TeamsStrava::Error, 'Unknown Message (404)')
    ...
    it 'clears unbragged_at' do
      ...
      allow(TeamsStrava::Bot.instance).to receive(:update_message).and_return(Struct.new(:id).new('1'))
      activity.rebrag!
  ...
  context 'unbrag!' do
    ...
    context 'a bragged message' do
      ...
      it 'deletes a previously sent message' do
        expect(TeamsStrava::Bot.instance).to receive(:delete_message).with(
      ...
      it 'ignores a previously delete message' do
        allow(TeamsStrava::Bot.instance).to receive(:delete_message)
    ...
    context 'an unbragged message' do
      ...
      it 'ignores a previously delete message' do
        expect(TeamsStrava::Bot.instance).not_to receive(:delete_message)
        expect(activity.unbrag!).to be_nil
      end
    end
  end
  ...
  context 'in time' do
    context 'miles' do
      ...
      it 'to_message' do
        expect(activity.to_message).to eq(
          {
            embeds: [
              {
                title: activity.name,
                url: "https://www.strava.com/activities/#{activity.strava_id}",
                description: "#{activity.user.teams_mention} 🥇 on Tuesday, February 20, 2018 at 10:02 AM\n\nGreat run!",
                image: {
                  url: "https://strata.playplay.io/api/maps/#{activity.map.id}.png"
                },
                fields: [
      ...
      context 'without an athlete' do
        ...
        it 'to_message' do
          expect(activity.reload.to_message).to eq(
            {
              embeds: [
                {
                  title: activity.name,
            ...
            embeds: [
              {
                title: activity.name,
                url: "https://www.strava.com/activities/#{activity.strava_id}",
                description: "#{activity.user.teams_mention} 🥇 on Wednesday, January 29, 2025 at 09:07 AM",
                fields: [
  ...
  context 'maps' do
    context 'without maps' do
      ...
      let(:user) { Fabricate(:user, team:) }
      let(:activity) { Fabricate(:user_activity, user:) }
      let(:embed) { activity.to_message[:embeds].first }
    ...
    context 'with thumbnail' do
      ...
      it 'to_message' do
        expect(embed.keys).not_to include :image
        expect(embed[:thumbnail][:url]).to eq "https://strata.playplay.io/api/maps/#{activity.map.id}.png"
      end
      ...
      it 'has an image' do
        ...
        expect(activity.map.proxy_image_url).to eq "https://strata.playplay.io/api/maps/#{activity.map.id}.png"
      end
    end
  end
        ...
        embeds: [
          {
            title: activity.name,
            url: "https://www.strava.com/activities/#{activity.strava_id}",
            description: "#{activity.user.teams_mention} 🥇 on Friday, February 16, 2018 at 06:52 AM",
            image: {
              url: "https://strata.playplay.io/api/maps/#{activity.map.id}.png"
            },
            fields: [
