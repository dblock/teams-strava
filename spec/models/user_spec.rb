...
describe User do
  context 'activated team' do
        ...
        context 'with bragged activities' do
          ...
          context 'latest activity' do
            ...
            it 'retrieves last activity details and rebrags it with updated description, device and image' do
              expect_any_instance_of(described_class).to receive(:update!) do |_instance, message, channel_message|
                expect(channel_message).to eq last_activity.channel_message
                expect(message[:embeds].first[:description]).to eq "#{user.teams_mention} 🥇 on #{last_activity.start_date_local_s}\n\ndetailed description"
                expect(message[:embeds].first[:fields]).to include(inline: true, name: 'Device', value: 'Strava iPhone App')
                expect(message[:embeds].any? { |embed| embed.dig(:image, :url) == 'https://dgtzuqphqg23d.cloudfront.net/Bv93zv5t_mr57v0wXFbY_JyvtucgmU5Ym6N9z_bKeUI-128x96.jpg' }).to be true
              end
              user.rebrag!
            end
    ...
    context 'team_owner?' do
      let!(:team) { Fabricate(:team, installer_id: 'installer-id') }

      it 'returns true if the user installed the app' do
        expect(Fabricate(:user, team:, user_id: team.installer_id)).to be_team_owner
      end

      it 'returns false by default' do
        expect(Fabricate(:user, team:)).not_to be_team_owner
      end
    end
    ...
    context 'with a max_activities_per_user_per_day limit and team timezone' do
      let!(:team) do
        Fabricate(
          :team,
          timezone: 'Pacific Time (US & Canada)',
          max_activities_per_user_per_day: 1
        )
      end
      let!(:user) { Fabricate(:user, team:) }
      let!(:already_bragged_activity) do
        Fabricate(
          ...
          user:,
          team:,
          bragged_at: Time.utc(2026, 3, 18, 23, 30, 0),
          start_date: 3.hours.ago
        )
      end
      let!(:pending_activity) { Fabricate(:user_activity, user:, team:, start_date: 2.hours.ago) }

      it 'uses the team timezone when applying the daily limit' do
        Timecop.freeze(Time.utc(2026, 3, 19, 3, 0, 0)) do
          expect_any_instance_of(UserActivity).not_to receive(:brag!)
          expect(user.brag!).to be_nil
          expect(pending_activity.reload.bragged_at).to be_nil
        end
    ...
    describe '#inform!' do
      let(:user) { Fabricate(:user, user_id: 'user-id', service_url: 'https://smba.trafficmanager.net/amer/') }

      it 'sends a message to the user channel' do
        expect(TeamsStrava::Bot.instance).to receive(:send_message)
          .with(user.channel_id, user.service_url, 'message')
          .and_return(Struct.new(:id).new('id'))

        expect(user.inform!('message')).to eq(message_id: 'id', channel_id: user.channel_id)
      end
    end

    describe '#left_team?' do
      let(:user) { Fabricate(:user) }

      it 'returns false when the user is still in the team conversation' do
        expect(TeamsStrava::Bot.instance).to receive(:member).with(user.team.conversation_id, user.user_id).and_return({})
        expect(user.left_team?).to be false
      end

      it 'returns true when Teams reports an unknown member' do
        allow(TeamsStrava::Bot.instance).to receive(:member)
          .with(user.team.conversation_id, user.user_id)
          .and_raise(TeamsStrava::Error, 'Unknown Member (404)')
        expect(user.left_team?).to be true
      end

      it 're-raises other Teams errors' do
        allow(TeamsStrava::Bot.instance).to receive(:member)
          .with(user.team.conversation_id, user.user_id)
          .and_raise(TeamsStrava::Error, 'Missing Access (403)')
        expect { user.left_team? }.to raise_error(TeamsStrava::Error, 'Missing Access (403)')
      end
    end
    ...
    describe '#dm_connect!' do
      ...
      it 'uses the default message' do
        expect(user).to receive(:dm!) do |message|
          expect(message).to be_a(Teams::Api::MessageActivity)
      ...
      it 'uses a custom message' do
        expect(user).to receive(:dm!) do |message|
          expect(message).to be_a(Teams::Api::MessageActivity)
          content = message.to_h.dig('attachments', 0, 'content')
          expect(content['body'].first).to include('text' => 'Please reconnect your account.')
          expect(content['actions'].first).to include('url' => url)
        end
  ...
  describe '#dm!' do
    ...
    let!(:team) { Fabricate(:team, tenant_id: 'tenant-id') }
    let!(:user) { Fabricate(:user, user_id: 'user-id', team:) }

    it 'sends a Teams DM' do
      expect(TeamsStrava::Bot.instance).to receive(:send_dm)
        .with(user.user_id, team.tenant_id, 'test')
        .and_return(Struct.new(:id, :conversation_id).new('activity-id', 'conversation-id'))
    ...
    it 'handles 403' do
      allow(TeamsStrava::Bot.instance).to receive(:send_dm)
        .with(user.user_id, team.tenant_id, 'test')
        .and_raise(TeamsStrava::Error, 'Cannot send messages to this user (403)')

      expect { user.dm!('test') }.to raise_error(TeamsStrava::Error, /403/)
    end

    it 'handles 400' do
      allow(TeamsStrava::Bot.instance).to receive(:send_dm)
        .with(user.user_id, team.tenant_id, 'test')
        .and_raise(TeamsStrava::Error, 'Bad Request (400)')

      expect { user.dm!('test') }.to raise_error(TeamsStrava::Error, /400/)
    end
  end
end
