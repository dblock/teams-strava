      ...
      module ParseDate
        ...
        def parse_date(date_time, guess = :first, now: Time.now)
          if (year = parse_year(date_time))
            ...
            if parsed.is_a?(Chronic::Span)
              ...
              raise TeamsStrava::Error, "Sorry, I don't understand '#{date_time}'."
            end
          end
        end
        ...
        def parse_date_expression(expression, now: Time.now)
          ...
          if expression.match?(/^between(\s)/i)
            ...
            raise TeamsStrava::Error, "Sorry, I don't understand '#{expression}'." unless dates.length == 2
            ...
            if expression.blank?
              ...
              if parsed.is_a?(Chronic::Span)
                ...
                raise TeamsStrava::Error, "Sorry, I don't understand '#{expression}'."
              end
            end
          end
