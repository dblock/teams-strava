class ActivityFields
  ...
  def self.parse_s(values)
    ...
    if errors.any?
      raise TeamsStrava::Error, "Invalid field#{'s' unless errors.one?}: #{errors.and}, possible values are #{ActivityFields.values.and}."
    end

    if fields.count > 1
      if fields.include?('None')
        raise TeamsStrava::Error, 'None cannot be used with other fields.'
      elsif fields.include?('Default')
        raise TeamsStrava::Error, 'Default cannot be used with other fields.'
      elsif fields.include?('All')
        raise TeamsStrava::Error, 'All cannot be used with other fields.'
      end
    end
