module ActivityMethods
  ...
  def embed_fields(channel_id = nil)
    ...
    activity_fields.each do |activity_field|
      ...
      when 'Max Heart Rate'
        fields << { inline: true, name: 'Max Heart Rate', value: max_heartrate_s } if max_heartrate&.positive?
      when 'PR Count'
        fields << { inline: true, name: 'PR Count', value: pr_count_s } if pr_count&.positive?
      when 'Calories'
      ...
      when 'Gear'
        fields << { inline: true, name: 'Gear', value: gear } if gear
      end
    end
