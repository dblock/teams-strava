class UserActivity < Activity
  ...
  def brag!
    ...
    if hidden?
      ...
      rc = user.inform!(to_message(channel_id))
      update_attributes!(bragged_at: Time.now.utc, unbragged_at: nil, channel_message: rc)
      rc
    end
  rescue TeamsStrava::Error => e
    raise unless User::DISABLE_SYNC_ERRORS.any? { |pattern| e.message =~ pattern }
  ...
  def rebrag!
    ...
    rc = user.update!(to_message(channel_message.channel_id), channel_message)
  ...
  def to_embed(channel_id = nil)
      ...
      if display_field?(ActivityFields::USER, channel_id) || display_field?(ActivityFields::DATE, channel_id)
        [
          if display_field?(ActivityFields::USER, channel_id)
            [user.teams_mention, display_field?(ActivityFields::MEDAL, channel_id) ? user.medal_s(type) : nil].compact.join(' ')
          end,
    ...
    result_fields = embed_fields(channel_id)
    result[:fields] = result_fields if result_fields&.any?
    result[:timestamp] = start_date&.utc&.iso8601
    result.merge!(user.athlete.to_embed_author) if user.athlete && display_field?(ActivityFields::ATHLETE, channel_id)
    result
  end

  def to_embeds(channel_id = nil)
    embeds = [to_embed(channel_id)]
    # photo may be displayed instead of the map already
    embeds.concat(photos.map(&:to_embed)) if display_field?(ActivityFields::PHOTOS, channel_id) && photos.any? && map&.has_image?
    embeds
  end
