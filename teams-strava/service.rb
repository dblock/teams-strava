module TeamsStrava
  class Service
    ...
    def self.url
      ENV.fetch('URL') { ENV['RACK_ENV'] == 'development' ? LOCALHOST : 'https://strata.playplay.io' }
    end
    ...
    def self.instance
      @instance ||= TeamsStrava::Service.new
    end
    ...
    def deactivate!(team)
      ...
      run_callbacks :error, team, e
      logger.error "#{team.team_name}: #{e.class}, #{e.message}, ignored."
